...
export function SaleForm({ onClose }: SaleFormProps) {
  const { menuItems, products, addSale } = useInventory()
  ...
  const updateSaleItem = (index: number, field: keyof SaleItem, value: string | number) => {
    const updatedItems = saleItems.map((item, i) => {
      if (i === index) {
        ...
        // Si se cambia el producto del menú, actualizar precio y nombre
        if (field === "menuItemId") {
        ...
        // Si se cambia la cantidad, recalcular total
        if (field === "quantity") {
  ...
  const calculateTax = () => {
    // IVA del 19% solo para algunos productos (puedes ajustar esta lógica)
